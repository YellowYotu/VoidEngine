...
public:
    ...
    bool start() {
        ...
        const std::string script = "$OutputEncoding=[Console]::OutputEncoding=[System.Text.UTF8Encoding]::new($false);"
            ...
            "'run' { if(!(Test-Path -LiteralPath '.\\build\\bin\\VoidApp.exe')){Write-Output 'Executable not found. Building first...'; void build}; if(Test-Path -LiteralPath '.\\build\\bin\\VoidApp.exe'){Write-Output 'Running VoidApp.exe...'; & '.\\build\\bin\\VoidApp.exe'} }"
